//bright varie de 0 à +infini, 1 = luminosité VecteurNormale
		...
		for (var x = 0; x < w; ++x) {
			...
			for (j = 1 - kernelSize; j < kernelSize; ++j) {
				...
				for (i = 1 - kernelSize; i < kernelSize; ++i) {
					...
					r += data[4 * ((y + j) * w + (x + i)) + 0] * kernel[Math.abs(j)][Math.abs(i)];
					g += data[4 * ((y + j) * w + (x + i)) + 1] * kernel[Math.abs(j)][Math.abs(i)];
					b += data[4 * ((y + j) * w + (x + i)) + 2] * kernel[Math.abs(j)][Math.abs(i)];
					a += data[4 * ((y + j) * w + (x + i)) + 3] * kernel[Math.abs(j)][Math.abs(i)];
				}
			}
			data[4 * (y * w + x) + 0] = r / kernelSum;
			data[4 * (y * w + x) + 1] = g / kernelSum;
			data[4 * (y * w + x) + 2] = b / kernelSum;
			data[4 * (y * w + x) + 3] = a / kernelSum;
		}
	}
