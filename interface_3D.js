...
case "3D_globale":	Wc_3D=Wc_3D_globale;
					Hc_3D=2*Wc_3D/3;
...
case "molette_R":	Wc_3D=Wc_molette_R;
					Hc_3D=Wc_3D;
...
x=x+(-dX)*k_px_3D;	y=y-dY*k_px_3D;	p=p-dP*k_px_3D;	
}
...
//Translation
if(flag!='pas_de_translation'){
x_p=x_p+Tx_3D;	y_p=y_p+Ty_3D;	p_p=p_p+Tp_3D;
}


return {x: x_p, y: y_p, p: p_p};
}
...
var Yt1 =  e.clientY - rect.top - root.scrollTop;
	...
	var Ax=temp2.x;	var Ay=temp2.y;	var Ap=temp2.p;	
		
	//B: point final de la souris
	//coordonées dans x,y,p
	temp2=XY2xyp(Xt1,Yt1);
	var Bx=temp2.x;	var By=temp2.y;	var Bp=temp2.p;
...
if(cvs_3d=="molette_R" || flag_mvt_3D=="rotation"){
	...
	Ax=toto.x;	Ay=toto.y;	Ap=toto.p;	

	var tata=Rotation3D(Bx_temp,By_temp,Bp_temp,"pas_de_translation");
	Bx=tata.x;	By=tata.y;	Bp=tata.p;	
	}


	//M: OM=OA v OB  ; |OM| = |OA|.|OB|.sin
	var Mx=Ay*Bp-Ap*By;	var My=Ap*Bx-Ax*Bp;	var Mp=Ax*By-Ay*Bx;			
	
	//Produit scalaire: |OA|.|OB|.cos
	var prod_scal=Ax*Bx+Ay*By+Ap*Bp;	

	//Normes des vecteurs
	var norme_OA=Math.sqrt(Ax*Ax+Ay*Ay+Ap*Ap);	var norme_OB=Math.sqrt(Bx*Bx+By*By+Bp*Bp);	var norme_OM=Math.sqrt(Mx*Mx+My*My+Mp*Mp);	
	...
	if(normalise!=0){
		...
		Mx=Mx*normalise;		My=My*normalise;		Mp=Mp*normalise;		
